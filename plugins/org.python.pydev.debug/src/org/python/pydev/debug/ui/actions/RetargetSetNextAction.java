...
public class RetargetSetNextAction extends RetargetAction {
	...
	class DebugContextListener implements IDebugContextListener {
		...
		protected void contextActivated(ISelection selection) {
			...
			if (selection instanceof IStructuredSelection) {
				...
				if (ss.size() == 1) {
					fTargetElement = (ISuspendResume) DebugPlugin.getAdapter(ss
							.getFirstElement(), ISuspendResume.class);
				}
			}
	...
	protected boolean canPerformAction(Object target, ISelection selection,
		...
		return fTargetElement != null &&
			((ISetNextTarget)target).canSetNextToLine(part, selection, fTargetElement);
	}
	...
	@Override
	protected void performAction(Object target, ISelection selection,
			IWorkbenchPart part) throws CoreException {

		boolean result = ((ISetNextTarget) target).setNextToLine(part,
