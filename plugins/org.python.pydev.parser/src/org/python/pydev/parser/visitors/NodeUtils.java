...
public class NodeUtils {
	...
	public static boolean isValidContextForSetNext(SimpleNode ast, int sourceLine, int targetLine) {
		...
		String targetFunctionName = NodeUtils.getContextName(targetLine + 1, ast);
		if (sourceFunctionName.equals(targetFunctionName)) {
			...
			if (sourceAST == null && targetAST != null){
				return false; // Source is outside loop and target is inside loop
			}
			if (sourceAST != null && targetAST != null){
	...
	/**
	 * Identify the for or while for a provided line number.
	 * 
	 * @param lineNumber
	 * @param ast
	 * @return
	...
	public static ASTEntry getLoopContextName(int lineNumber, SimpleNode ast) {
		...
		if (ast != null) {
			...
			// ASTEntry loopContext = null;
			ArrayList<ASTEntry> forAndWhileList = new ArrayList<ASTEntry>();
			EasyASTIteratorWithLoop visitor = EasyASTIteratorWithLoop
					.create(ast);
			Iterator<ASTEntry> forAndWhileIterator = visitor
					.getForAndWhileIterator();
			while (forAndWhileIterator.hasNext()) {
				ASTEntry entry = forAndWhileIterator.next();
				if ((entry.node.beginLine) < lineNumber
						&& entry.endLine >= lineNumber) {
					forAndWhileList.add(entry);
					if (entry.node.beginLine > highestBeginLine) {
						highestBeginLine = entry.node.beginLine;
					}
				}
			}
			Iterator<ASTEntry> forAndWhileListIterator = forAndWhileList
					.iterator();
			while (forAndWhileListIterator.hasNext()) {
				ASTEntry astEntry = forAndWhileListIterator.next();
